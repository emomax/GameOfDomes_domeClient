...
void myInitOGLFun()
{
	...
	nodeTracker->setTrackNode(mModel); //cookbook bit
	if (mModel.valid())
	{
...
void myPreSyncFun()
{
	if (gEngine->isMaster())
	{
		...
		if (arrowButtons[LEFT])
			turn_x.setVal(turn_x.getVal() - (navigation_speed * gEngine->getDt()));
...
void myPostSyncPreDrawFun()
{
	...
	//mSceneTrans->postMult(osg::Matrix::rotate(1.0, turn_x.getVal(), -0.1, 0.0));
	...
	mViewer->setCameraManipulator(nodeTracker.get());
...
void myEncodeFun()
{
	...
	sgct::SharedData::instance()->writeDouble(&turn_x);
...
void myDecodeFun()
{
	...
	sgct::SharedData::instance()->readDouble(&turn_x);
