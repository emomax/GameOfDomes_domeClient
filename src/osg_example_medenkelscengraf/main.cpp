...
sgct::SharedDouble side(-2.0);	//Eget test (sidoförflyttning)
...
const double navigation_speed = 2.0;
...
void myInitOGLFun()
{
	...
	mModel2Trans->preMult(osg::Matrix::rotate(glm::radians(-90.0f),
											 0.0f, 1.0f, 0.0f));
	mModel2Trans->preMult(osg::Matrix::translate(0.0, 0.0, 3.0));
	...
	if (mModel.valid())
	{
		...
		mModel1Trans->postMult(osg::Matrix::translate( -tmpVec ) );
		mModel2Trans->postMult(osg::Matrix::translate(10.0, 10.0, 0.0));
		mModel1Trans->postMult(osg::Matrix::scale( 1.0f/bb.radius(), 1.0f/bb.radius(), 1.0f/bb.radius() ));
		mModel2Trans->postMult(osg::Matrix::scale(1.0f / bb.radius(), 1.0f / bb.radius(), 1.0f / bb.radius()));
...
void myPreSyncFun()
{
	if( gEngine->isMaster() )
	{
		...
		if (arrowButtons[LEFT])
			side.setVal(side.getVal() + (navigation_speed * gEngine->getDt()));
...
void myPostSyncPreDrawFun()
{
	...
	mSceneTrans->setMatrix(osg::Matrix::identity());
	...
	mModel1Trans->postMult(osg::Matrix::scale(1.0f / 10.0f, 1.0f / 10.0f, 1.0f / 10.0f));
	mModel1Trans->postMult(osg::Matrix::rotate(glm::radians(-90.0f), 1.0f, 0.0f, 0.0f));
	mModel1Trans->postMult(osg::Matrix::rotate( glm::radians(curr_time.getVal() * 8.0), 0.0, 1.0, 0.0));
	mSceneTrans->postMult(osg::Matrix::translate(side.getVal(), 0.0, dist.getVal()));
...
void myEncodeFun()
{
	...
	sgct::SharedData::instance()->writeDouble(&side);
...
void myDecodeFun()
{
	...
	sgct::SharedData::instance()->readDouble(&side);
